...
using Comtec.AS400XML.Output.Data;
...
namespace Comtec.AS400XML {
    public class AS400XmlParser {
        ...
        public int DeserializeFilesInDirectory<T>(string directory) where T : ScreenXmlRoot {
            ...
            foreach (string xmlFilePath in xmlFiles) {
                try {
                    counter++;
                    LogHelper.WriteInformation(MethodBase.GetCurrentMethod(), $"File {xmlFilePath} Started");

                    XmlFileModel xmlFileModel = DeserializeFilePath<T>(xmlFilePath);

                    AS400XmlContext as400XmlContext = new AS400XmlContext();
                    if (as400XmlContext.XmlFiles.Count(x => x.Hash == xmlFileModel.Hash) == 0) {
                        as400XmlContext.XmlFiles.Add(xmlFileModel);
                    ...
                    LogHelper.WriteFatal(MethodBase.GetCurrentMethod(), $"An error occurred while processing file {xmlFilePath}: {ex.Message}");
                }
            }
        ...
        public XmlFileModel DeserializeFilePath<T>(string xmlFilePath) where T : ScreenXmlRoot {
            XmlFileModel xmlFileModel = new XmlFileModel(xmlFilePath, xmlFilePath, xmlFilePath);

            string fileContent = File.ReadAllText(xmlFilePath);
            AS400XmlOutputModel as400XmlOutputModel = DeserializeString<T>(fileContent);
            xmlFileModel.ScreenXmlRoot = as400XmlOutputModel.ScreenXmlRoot;
            xmlFileModel.Hash = HashHelper.OneWayHash(xmlFileModel.XmlFilePath + xmlFileModel.JsonZip);

            return xmlFileModel;
        }

        public AS400XmlOutputModel DeserializeString<T>(string xmlString) where T : ScreenXmlRoot {
            ...
            LogHelper.WriteInformation(MethodBase.GetCurrentMethod(), "Xml import finished successfully");
        ...
        private void Serializer_UnknownAttribute(object? sender, XmlAttributeEventArgs e) {
            ...
            if (!_attributes[className].ContainsKey(e.Attr.Name)) {
                ...
                _attributes[className][e.Attr.Name].AppendLine($"    [XmlAttribute(\"{e.Attr.Name}\")]");
                _attributes[className][e.Attr.Name].AppendLine($"    [MaxLength(1000)]\r\n");
